...
result <- sleep %>%
  mutate(
    Cal_greater_than_0 = Cal > 0,
    Alc_greater_than_0 = Alc > 0
  ) %>%
  group_by(Cal_greater_than_0, Alc_greater_than_0) %>%
...
numeric_cols <- c("TTS", "TST", "TBT", "Run", "SleepEfficiency")
...
```{r}
# 6. Plot over time to look for trends
ggplot(sleep, aes(x = Date, y = TST)) +
  geom_line(color = "steelblue") +
  labs(title = "Total Sleep Time over Time",
       x = "Date",
       y = "TST (minutes)")
