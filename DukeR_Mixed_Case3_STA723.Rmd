...
model_mixed <- lmer(SleepEfficiency ~ TTS + Awake_time + Bed_hour + Cal + Alc + Run + (1 | Segment), data = sleep)
...
BAYESIAN MODEL: (with just TBT and Month and Segment)
...
# Fit Bayesian Hierarchical Model with Month as Fixed Effect
bayesian_model <- brm(
  formula = SleepEfficiency ~ TBT + Month + (1 | Segment), 
  data = sleep,
...
bayesian_model_full <- brm(
  formula = SleepEfficiency ~ TBT + Awake_time + Run + Cal + Alc + Month + (1 | Segment), 
  data = sleep,
  ...
  iter = 4000, warmup = 2000, chains = 4, cores = 4,  # MCMC settings
